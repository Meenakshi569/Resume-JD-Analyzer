...
const Home = () => {
    ...
    const handleGenerateReport = async () => {
        ...
        if (!jobDescription.trim() || (!resumeFile && !selfDescription.trim())) {
            window.alert("Please provide a job description and either upload a resume or enter a self description.")
            return
        }
    ...
    const handleDrop = (e) => {
        ...
        if (!allowedTypes.includes(file.type) && !file.name.toLowerCase().endsWith('.docx')) {
            window.alert('Please upload a valid PDF or DOCX file.')
            return
        }
                    ...
                    <div className='profile-sidebar__content'>
                        {reports.length === 0 ? (
                            ...
                            <ul className='reports-list'>
                                {reports.map(report => (
                                    <li key={report._id} className='report-item' onClick={() => { navigate(`/interview/${report._id}`); setShowProfile(false); }}>
                    ...
                    <div className='panel panel--right'>
                        ...
                        <div className='upload-section'>
                            ...
                            >
                                ...
                                {selectedFile ? (
                                    <p className='dropzone__title'>Selected: {selectedFile.name}</p>
                                ) : (
                                    <>
